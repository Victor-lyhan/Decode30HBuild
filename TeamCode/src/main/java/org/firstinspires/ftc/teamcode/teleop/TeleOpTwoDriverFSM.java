...
public class TeleOpTwoDriverFSM extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            //FSM controls — X to start, Back to cancel
            if (gamepad2.back) {
                intakeFSM.cancel();
                stopSubsystems();
            }
            if (!intakeFSM.isRunning() && gamepad2.x) {
                intakeFSM.start();
            }
            // Update FSM each loop (controls claw/elevator/intake only)
            intakeFSM.update();
            ...
            if (!launchFSM.isRunning() && gamepad2.b) {
                launchFSM.start();
            }
            // Update FSM each loop (controls claw/elevator/intake only)
            launchFSM.update();
